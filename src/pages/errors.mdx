...
<Note>
  ...
  your code before contacting IWDR support.
</Note>
...
Here is a list of the different categories of status codes returned by the IWDR API. Use these to understand if a request was successful.
...
<Row>
  <Col>
    ...
    Whenever a request is unsuccessful, the IWDR API will return an error response with an error type and message. You can use this information to understand better what has gone wrong and how to fix it. Most of the error messages are pretty helpful and actionable.

    Here is a list of the two error types supported by the IWDR API — use these to understand what you have done wrong.
  ...
  <Col>
    ...
    ```bash {{ title: "Error response" }}
    {
      ...
      "documentation_url": "https://IWDR.chat/docs/errors/api_error"
    }
    ```
