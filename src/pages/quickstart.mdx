...
This guide is aimed to help an organization get started with using the IWDR API.
It contains important information about mapping your organization data,
authenticating API requests, handling errors, and more.

## Who is this for

- IT staff with database experience, including import/export of data, mapping, and a good understanding of field names
- Organizations with IT capability and sufficient data volume that strongly desire to reduce dual entry

## Getting Started
Firstly, you must already be an IWDR subscriber. If you aren’t, <Button href="https://www.iwdr.org/enrollment-request/" target="_blank" variant="text" className="fill-emerald-500">you can enroll here</Button>.
1. **Submit an API Application.** You can access our API documentation and guide here: https://api-docs.iwdr.org/  Log in with your existing IWDR details and you will see the API Access Application link in the top right corner of the page.
    - Once your application is submitted, we will contact you to arrange a meeting and discuss your needs, migration requirements, your database type, a timeframe, and costs.
...
- **Your oganizations responsibilites**:
  ...
  - If you are not already an IWDR subscriber,  <Button href="https://www.iwdr.org/enrollment-request/" target="_blank" variant="text" className="fill-emerald-500">you must enroll first</Button>.
  - You must then also apply your organization to become an IWDR API user in order to obtain an authentication token/API key. You can apply
    for access [here](/token-application). You will need to confirm your specific requirements from this process, so we can be sure your needs are met.
  ...
  - Maintain these data requirements moving forward - the API will not parse data where the IWDR DogID and owner ID do not match
    ...
    depending on the data type. However, some types of bad data such as incorrect birthdates etc. can not be detected. {{ className: 'text-red-500' }}
...
- **IWDR will provide**:
  ...
  - Use of the IWDR Connector middleware program. Written in python this program can aid your organization with data mapping and
    transfer by using structured mapping files alongside a Command Line Interface (CLI) to create a seamless connection between
