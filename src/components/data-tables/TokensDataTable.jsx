...
export default function TokensDataTable() {
    ...
    const deleteToken = (id) => {
        setLoading(true);
        axios
            .delete(`/api/tokens/${id}`, { headers: { Authorization: 'Bearer ' + session?.user?.access_token } })
            .then((res) => {
                if (!res.status === 204) {
                    serverErrorAlert();
                }
    ...
    const headers = [
        ...
        {
            text: 'Date Last Used',
            component: (item) => (item.last_used_at ? new Date(item.last_used_at).toLocaleString() : 'N/A'),
        },
    ];

    const noDataMsg = 'You have not created any tokens yet. Click the create token button to make a new one.';
    ...
    return (
        <>
            ...
            >
                <TextField value={newToken} readonly={true} className="rounded-r-none" copyable={true} />
            </ActionPanel>
            <div className="sm:item-center sm:flex">
                <div className="sm:flex-auto">
                    <p className="mt-2 text-sm">A list of all tokens currently active for your organization.</p>
                </div>
