...
function CopyButton({ code }) {
    ...
    return (
        <button
            type="button"
            className={clsx(
                'group/button absolute top-3.5 right-4 overflow-hidden rounded-full py-1 pl-2 pr-3 text-2xs font-medium opacity-0 backdrop-blur transition hover:opacity-100 focus:opacity-100 group-hover:opacity-100',
                copied
...
function CodePanel({ tag, label, code, children, className }) {
    let child = Children.only(children);

    return (
        <div className={clsx(className, 'group dark:bg-white/2.5')}>
    ...
    return (
        <CodeGroupContext.Provider value={true}>
            ...
            >
                ...
                <CodeGroupPanels {...props} className={clsx(props.className, 'max-h-[350px] overflow-y-scroll')}>
                    {children}
                </CodeGroupPanels>
            </Container>
