        ...
        rows: [
            {
                ...
                value: appData.data?.project_current_storage_breed_info?.map((item) => item.label).join(', '),
            },
            {
                label: 'Diagnoses',
                field: 'project_current_storage_diagnoses',
                value: appData.data?.project_current_storage_diagnoses?.map((item) => item.label).join(', '),
            },
            {
                label: 'Behavior Checklists',
                field: 'project_current_storage_bcls',
                value: appData.data?.project_current_storage_bcls?.map((item) => item.label).join(', '),
            },
        ],
    };
        ...
        rows: [
            {
                ...
                value: appData.data?.api_usage_dog_info_puppies?.map((item) => item.label).join(', '),
            },
            {
                label: "Basic Dog Information- Updates on dog's names, status, death dates, etc",
                field: 'api_usage_dog_info_ancestors',
                value: appData.data?.api_usage_dog_info_ancestors?.map((item) => item.label).join(', '),
            },
            {
                label: "Dog's Status History",
                field: 'api_usage_status_history',
                value: appData.data?.api_usage_status_history?.map((item) => item.label).join(', '),
            },
            {
                label: 'Behavior Checklists',
                field: 'api_usage_bcls',
                value: appData.data?.api_usage_bcls?.map((item) => item.label).join(', '),
            },
            {
                label: 'General Health Diagnoses',
                field: 'api_usage_health_diagnoses',
                value: appData.data?.api_usage_health_diagnoses?.map((item) => item.label).join(', '),
            },
            {
                label: 'Genetic Test Results',
                field: 'api_usage_genetic_test_results',
                value: appData.data?.api_usage_genetic_test_results?.map((item) => item.label).join(', '),
            },
            {
                label: 'Weight',
                field: 'api_usage_weights',
                value: appData.data?.api_usage_weights?.map((item) => item.label).join(', '),
            },
            {
                label: "Estrus's and Litter's",
                field: 'api_usage_estrus_litter',
                value: appData.data?.api_usage_estrus_litter?.map((item) => item.label).join(', '),
            },
            {
                label: 'Estrus Details',
                field: 'api_usage_estrus_details',
                value: appData.data?.api_usage_estrus_details?.map((item) => item.label).join(', '),
            },
            {
                label: 'Laboratory Tests',
                field: 'api_usage_lab_tests',
                value: appData.data?.api_usage_lab_tests?.map((item) => item.label).join(', '),
            },
            {
                label: 'Vaccines',
                field: 'api_usage_vaccines',
                value: appData.data?.api_usage_vaccines?.map((item) => item.label).join(', '),
            },
            {
                label: 'Annual Health Survey',
                field: 'api_usage_health_survey',
                value: appData.data?.api_usage_health_survey?.map((item) => item.label).join(', '),
            },
        ],
    };
